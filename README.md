...
A daily public Actions run at 16:11 UTC checks **over 5,000 explicitly mapped operator avatars, skill/base-skill icons, material icons and enemy icons** against `yuanyan3060/ArknightsGameResource`. `asset-source-map.json` records exact upstream paths and blob hashes; accepted existing images form the initial baseline and are only re-encoded when their upstream blob changes. Operator base-skill discovery reads EN then CN building tables, while combat skills join character-table skill IDs to skill-table icon IDs so shared generic icons do not depend on filename guesses. Use `scripts/build_source_map.py` with public EN game-data/resource snapshots to extend exact mappings, then review the diff; add manually verified paths for other categories. Operator discovery maps a new operator's portraits, skill icons and token by character ID, and enemy discovery maps each `arkpedia-data` enemy record's own `icon` path to `enemy/<id>.png`. Both skip any file `asset-manifest.json` already lists, so hand-added art joins the map only by review (`build_source_map.py` for skill, base-skill and material icons; a manual map row for portraits). A map row without `sourceBlob` whose file the manifest already lists, such as a manual row or one the app's catalogue import wrote with its file, adopts the current upstream blob and keeps the file as it is; only an unlisted target is fetched. From then on the file is the mirror's and an upstream change re-encodes it, so a hand correction to a mapped file must remove its map row in the same commit. Unmapped banners, maps, UI assets and manual captures are preserved; this job does not pretend to discover their sources automatically.
...
The content release in the website repository (`Prepare content release`) also pushes here, fast-forward only, with a deploy key scoped to this repository. Its `catalogue-assets` stage imports a new operator's portraits, skill, base-skill and material icons and key art (`Import newly discovered catalogue media`); its `staged-art` stage publishes the art that release's importers fetched and nothing here had yet (`Publish art the content refresh staged`): a new event's stage renders from the CN mirror's `map/`, Integrated Strategies, Contingency Contract and Reclamation Algorithm renders, nation crests, drop icons (`item-icons/`), event reward and furniture icons (`material-icons/`) and Monster Siren album covers (`album-covers/`). Every file it adds gets a manifest row with its dimensions and a row in `sources/catalogue-imports.json` naming the URL it came from, the SHA-256 of the bytes fetched and, for staged art, the importer that fetched it. A file from `yuanyan3060/ArknightsGameResource` at the commit that release pinned also gets a map row with the blob of those exact bytes and the width its folder keeps, so the mirror sync above re-fetches it only when upstream changes it. The sync writes a mapped file in the format its name says (an `item-icons/*.png` stays a PNG) and keeps the game's circled item and material icons at their native 183px. A push that loses a race to one of this repository's own jobs is rebuilt on the new head and pushed again; it never overwrites a commit.
